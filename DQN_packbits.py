...
# TODO:
# Reset thing might be messing up?

class TimeLogger:
    def __init__(self, avg_size):
        self.avg_size = avg_size
        
        self.avg_memory = deque()
        for i in range(self.avg_size):
            self.avg_memory.append(0)
        
    def add(self, time):
        self.avg_memory.append(time)
        
        if len(self.avg_memory) > self.avg_size:
            self.avg_memory.popleft()
            
    def get_avg(self):
        avg = 0
        for i in range(self.avg_size):
            avg += self.avg_memory[i]
        avg = avg / self.avg_size
...
class Q_Net:
    ...
    def train_network(self):
        ...
        # Unpack bits
        state_batch_unpacked = np.unpackbits(state_batch, axis=1)        
        next_state_batch_unpacked = np.unpackbits(next_state_batch, axis=1)
        
        state_batch_unpacked = state_batch_unpacked[:,0:84,:,:]
        next_state_batch_unpacked = next_state_batch_unpacked[:,0:84,:,:]
        
        state_batch = state_batch_unpacked
        next_state_batch = next_state_batch_unpacked  
...
#            plt.gray()
#            plt.subplot(2,4,i+1)
#            plt.imshow(state_batch[15][:,:,i])
#            
#        #plt.figure(2)
#        for i in range(4):
#            plt.gray()
#            plt.subplot(2,4,4+i+1)
#            plt.imshow(state_batch[16][:,:,i])
#        
#        input()
    ...
    def add_to_replay_memory(self, state, action, reward, next_state, terminal):
        ...
        # Pack bits for more efficient storage
        state_packed = np.packbits(state, axis=0)
        next_state_packed = np.packbits(next_state, axis=0)
        
        #self.replay_memory.append((state, action, reward, next_state, terminal))
        self.replay_memory.append((state_packed, action, reward, next_state_packed, terminal))
...
for i in range(NUM_EPISODES):    
    ...
    print("AVERAGE TIME PER EPISODE: " + str(round(timelogger.get_avg(),2)) + " (" + str(round(time_elapsed,2)) + ")")
    
    if process.memory_info().rss / 1000000.0 > MAXIMUM_MEMORY:
        print("\nWARNING: MEMORY USAGE HIGHER THAN " + str(MAXIMUM_MEMORY) + " MB")        
        REPLAY_MEMORY_LENGTH = rep_mem_len       
        print("REPLAY MEMORY LENGTH CAPPED AT " + str(REPLAY_MEMORY_LENGTH))
